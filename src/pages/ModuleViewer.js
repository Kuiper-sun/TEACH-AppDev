...
const ModuleViewer = () => {
    const { moduleId } = useParams(); // Use dynamic moduleId from URL
    const navigate = useNavigate();
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(10);

    useEffect(() => {
        // Simulate fetching module data based on moduleId
        if (moduleId) {
            console.log(`Fetching data for module: ${moduleId}`);
            // Fetch module details, like title, total pages, etc., from API
        }
    }, [moduleId]);
        ...
        <div className="flex h-screen bg-blue-400">
            ...
            <div className="flex-1 bg-white rounded-tl-3xl p-8">
                ...
                <div className="flex items-center mb-6 text-gray-600">
                    ...
                    <span className="mx-2">/</span>
                    <span className="mr-2 cursor-pointer hover:text-blue-600" onClick={() => navigate('/modules')}>Modules</span>
                    <span className="mx-2">/</span>
